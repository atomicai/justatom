...
from farm.modeling.wordembedding_utils import EMBEDDING_VOCAB_FILES_MAP, load_from_cache, run_split_on_punc
...
class EmbeddingTokenizer(PreTrainedTokenizer):
    ...
    def from_pretrained(cls, pretrained_model_name_or_path, **kwargs):
        ...
        if pretrained_model_name_or_path in EMBEDDING_VOCAB_FILES_MAP["vocab_file"]:
            ...
            temp = open(str(Path(pretrained_model_name_or_path) / "language_model_config.json"), encoding="utf-8").read()  # noqa: SIM115
    ...
    def save_pretrained(self, vocab_path):
        ...
        index = 0
        if os.path.isdir(vocab_path):  # noqa: SIM108
...
def truncate_sequences(
    seq_a, seq_b, tokenizer, max_seq_len, truncation_strategy="longest_first", with_special_tokens=True, stride=0
):
    """
