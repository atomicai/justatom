...
class EVAL_OPTS:
    def __init__(
        self, data_file, pred_file, out_file="", na_prob_file="na_prob.json", na_prob_thresh=1.0, out_image_dir=None, verbose=False
    ):
...
def parse_args():
    ...
    parser.add_argument(
        "--na-prob-thresh", "-t", type=float, default=1.0, help='Predict "" if no-answer probability exceeds this (default = 1.0).'
    )
    parser.add_argument(
        ...
        for p in article["paragraphs"]:
            for qa in p["qas"]:
                ...
                if not gold_answers:
                    ...
                    gold_answers = [""]
                if qid not in preds:
                    print("Missing prediction for %s" % qid)  # noqa: UP031
                    continue
                a_pred = preds[qid]
                # Take max over all gold answers
                exact_scores[qid] = max(compute_exact(a, a_pred) for a in gold_answers)
                f1_scores[qid] = max(compute_f1(a, a_pred) for a in gold_answers)
                labels_and_preds[qid] = {"label": list(set(gold_answers)), "pred": a_pred}
    json.dump(labels_and_preds, open("predictions_and_labels.json", "w"))  # noqa: SIM115
...
def merge_eval(main_eval, new_eval, prefix):
    for k in new_eval:
        main_eval["%s_%s" % (prefix, k)] = new_eval[k]  # noqa: UP031
...
def histogram_na_prob(na_probs, qid_list, image_dir, name):
    ...
    plt.title("Histogram of no-answer probability: %s" % name)  # noqa: UP031
    plt.savefig(os.path.join(image_dir, "na_prob_hist_%s.png" % name))  # noqa: UP031
    plt.clf()
...
def find_best_thresh(preds, scores, na_probs, qid_to_has_ans):
    ...
    best_thresh = 0.0
    qid_list = sorted(na_probs, key=lambda k: na_probs[k])
    for i, qid in enumerate(qid_list):  # noqa: B007
        if qid not in scores:
            continue
        if qid_to_has_ans[qid]:
            diff = scores[qid]
        else:
            if preds[qid]:  # noqa: SIM108
                diff = -1
            else:
                diff = 0
        cur_score += diff
