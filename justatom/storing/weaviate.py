...
from justatom.etc.filter import FilterType, LogicalFilterClause
from justatom.etc.format import convert_date_to_rfc3339, get_batches_from_generator
from justatom.etc.errors import DocumentStoreError, IError

from justatom.etc.lazy_imports import LazyImport

with LazyImport("Run 'pip install weaviate-client'") as weaviate_import:
    import weaviate
...
class KeywordDocumentStore(INNDocStore):
    """
    ...
    def normalize_embedding(emb: np.ndarray) -> None:
        ...
        if len(emb.shape) == 1:
            _normalize_embedding_1D(emb)
        # 2D matrix
        else:
            _normalize_embedding_2D(emb)
...
class IWeaviateDocStore(KeywordDocumentStore):
    """
