...
def get_model_type_list(models, type="vlm"):
    ...
    if type == "vlm":
        submodules = [
            ...
            if os.path.isdir(os.path.join(models_dir, item)) and not item.startswith(".") and item != "__pycache__"
        ]
        return submodules
    else:
...
def load_tools_config():
    with open(str(TOOLS_PATH / "config.json")) as file:
...
def apply_lm_chat_template(tokenizer: Any, chat_messages: List[Dict], request: ChatCompletionRequest) -> str:
    if tokenizer.chat_template is not None and hasattr(tokenizer, "apply_chat_template"):
        if "firefunction-v2" in request.model:
            ...
            return tokenizer.apply_chat_template(
                chat_messages,
                functions=json.dumps([tool.model_dump() for tool in request.tools], indent=4),
                datetime=now,
                tokenize=False,
            )
        else:
...
def handle_function_calls(output: str, request):
    ...
    if json_match:
        try:
            ...
            output = re.sub(r'\{.*"tool_calls":\s*\[.*\].*\}', "", output, flags=re.DOTALL).strip()
    ...
    # Check for function calls in both old and new XML formats
        ...
        try:
            ...
            for function_name, args_str in function_calls:
                ...
                tool_calls.append(
                    ToolCall(
                        id=f"call_{os.urandom(4).hex()}",
                        function=FunctionCall(name=function_name, arguments=json.dumps(args)),
                    )
                )

            # Try parsing new XML format
            invoke_blocks = re.findall(r"<invoke>(.*?)</invoke>", output, re.DOTALL | re.IGNORECASE)
            for block in invoke_blocks:
                tool_name = re.search(r"<tool_name>(.*?)</tool_name>", block, re.IGNORECASE)
                parameters = re.findall(r"<(\w+)>(.*?)</\1>", block, re.IGNORECASE)

                if tool_name:
                    args = {param[0].lower(): param[1] for param in parameters if param[0].lower() != "tool_name"}
                    tool_calls.append(
                        ToolCall(
                            id=f"call_{os.urandom(4).hex()}",
                            function=FunctionCall(name=tool_name.group(1), arguments=json.dumps(args)),
                        )
                    )
        ...
        try:
            ...
            if functools_match:
                ...
                output = re.sub(r"functools\[.*?\]", "", output, flags=re.DOTALL).strip()
        except Exception as e:
...
def lm_generate(
    ...
    **kwargs,
    ...
    stop_words_id = tokenizer._tokenizer(stop_words)["input_ids"][0] if stop_words else None
    ...
    for (token, logprobs), n in zip(
        ...
        if token == tokenizer.eos_token_id or (stop_words_id and token in stop_words_id):
            break
...
def lm_stream_generator(model, model_name, tokenizer, prompt, max_tokens, temperature, **kwargs):
    stop_words = kwargs.pop("stop_words", [])
