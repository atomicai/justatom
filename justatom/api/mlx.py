...
from justatom.api.types.chat.chat_completion import (
    ChatCompletionKeywordsRequest,
    ChatCompletionKeywordsResponse,
    ChatCompletionRequest,
    ChatCompletionResponse,
    ChatMessage,
)
from justatom.api.types.model import SupportedModels
from justatom.running.prompt import KEYPromptRunner
...
try:
    ...
    from justatom.api.tools.all import (
        MODEL_REMAPPING,
        MODELS,
...
async def chat_completion(request: ChatCompletionRequest):
    ...
    logger.info(f"Got request model param = {request.model}")
...
@app.post("/v1/chat/completions/keywords", response_model=ChatCompletionKeywordsResponse)
async def chat_completion_as_keywords(request: ChatCompletionKeywordsRequest):
    if not MLX_AVAILABLE:
        raise HTTPException(status_code=500, detail="MLX library not available")
    stream = request.stream
    logger.info(f"Got request model param = {request.model}")
    ...
    tokenizer = model_data["tokenizer"]
    pr_key_runner = KEYPromptRunner(
        system_prompt=f"""
        Ты - эксперт лингвиcт, специализирующийся на объяснении трудной информации на {request.source_language} языке.\n\n
        Твоя задача выделить ключевые слова из вселенной, которая может быть книгой, фильмом или компьютерной игрой и выдать набор ключевых слов или фраз. Обрати внимание, что каждая фраза должна быть подстрокой параграфа.\n\n
        """
    )
    chat_messages = list(flatten([pr_key_runner.prompt(content=msg.content, title=msg.title) for msg in request.messages]))
    prompt = apply_lm_chat_template(tokenizer, chat_messages, request)
    ...
    after_finalize_output: dict = pr_key_runner.finalize(
        content=chat_messages[-1]["content"], raw_response=repair_json(output), as_json_string=False
    )
    response = ChatCompletionKeywordsResponse(
        id=f"chatcmpl-{os.urandom(4).hex()}",
        created=int(time.time()),
        choices=[
            {
                "index": 0,
                "message": {"role": "assistant", "content": after_finalize_output},
            }
        ],
    )
    return response
    # Parse the output to check for function calls
    # response = handle_function_calls(after_finalize_output, request)
    # response["choices"][0]["message"]["content"] = json.loads(response["choices"][0]["message"]["content"])
    # return response
...
def main():
    ...
    parser.add_argument("--port", type=int, default=8228, help="Port to run the server on")
    parser.add_argument(
        "--reload",
        type=bool,
        default=False,
