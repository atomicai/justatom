...
class IMetrics(UserDict):
    ...
    metrics = dict(
        ...
        F1Micro=functools.partial(tm.F1Score, task="multiclass", average="micro"),
        F1Macro=functools.partial(tm.F1Score, task="multiclass", average="macro"),
    )

    def __init__(self, metrics, num_classes):
        super().__init__()
        for metric in metrics:
            super().__setitem__(metric, self.metrics[metric](num_classes=num_classes))


__all__ = ["IAdditiveMetric", "IMetrics"]
