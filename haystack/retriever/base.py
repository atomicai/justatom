...
from farm.utils import get_iterator
...
class BaseRetriever(BaseComponent):
    ...
    def eval(
        ...
        if open_domain:
            for question, gold_answers in get_iterator(question_label_dict.items(), show_bar=True, in_notebook=in_notebook):
        ...
        # Option 2: Strict evaluation by document ids that are listed in the labels
            ...
            for question, gold_ids in get_iterator(question_label_dict.items(), show_bar=True, in_notebook=in_notebook):
                retrieved_docs = timed_retrieve(question, top_k=top_k, index=doc_index)
                if return_preds:
                    predictions.append({"question": question, "retrieved_docs": retrieved_docs})
                # check if correct doc in retrieved docs
