...
class HaystackDocumentStore:
    ...
    def get_document_store(self):
        if self._store_type == 'ElasticsearchDocumentStore':
            es = Elasticsearch(['http://localhost:9200/'], verify_certs=True)
            es.indices.delete(index='document', ignore=[400, 404])
            return ElasticsearchDocumentStore(**self._kwargs)
        elif self._store_type == 'FAISSDocumentStore':
            return FAISSDocumentStore(**self._kwargs)
        else:
            raise ValueError(f'Unknown store of type \"{self._store_type}\" and could not be initialized')
...
def add_is_impossible(squad_data: dict, json_file_path: Path):
    ...
    with open(new_path, "w", encoding='utf-8-sig') as filo:
...
def save_dataset(iter_dpr: Iterator, dpr_output_filename: Path,
    ...
    for path, set_iter in dataset_splits.items():
        with open(path, "w", encoding='utf-8-sig') as json_ds:
...
def get_hard_negative_contexts(retriever: BaseRetriever, question: str, answers: List[str],
    ...
    retrieved_docs = retriever.retrieve(query=question, top_k=min(30, 30 * n_ctxs), index="document")
