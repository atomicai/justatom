...
def eval_data_from_json(filename: Union[str, Path], max_docs: Union[int, bool] = None, preprocessor: PreProcessor = None, open_domain: bool =False) -> Tuple[List[Document], List[Label]]:
    """
...
def eval_data_from_jsonl(filename: str, batch_size: Optional[int] = None,
    ...
    with open(filename, "r", encoding='utf-8-sig') as file:
        for document in file:
            ...
            cur_docs, cur_labels, cur_problematic_ids = _extract_docs_and_labels_from_dict(document_dict, preprocessor, open_domain=open_domain)
            docs.extend(cur_docs)
            labels.extend(cur_labels)
            problematic_ids.extend(cur_problematic_ids)
            ...
            if batch_size is not None:
                if len(docs) >= batch_size:
                    if len(problematic_ids) > 0:
                        logger.warning(f"Could not convert an answer for {len(problematic_ids)} questions.\n"
                                       f"There were conversion errors for question ids: {problematic_ids}")
                    yield docs, labels, problematic_ids
                    docs = []
                    labels = []
                    problematic_ids = []
        ...
        for qa in paragraph["qas"]:
            if not qa.get("is_impossible", False):
                for answer in qa["answers"]:
                    # TODO: Maybe we add here lower() ?
                    ans = answer["text"]
                    while ans.endswith(tuple(string.punctuation)):
                        ans = ans[0:-1]
                    cur_ans_start = None
                    ...
                    if open_domain:
                        cur_ans_start = answer.get("answer_start", cur_doc.text.find(ans))
                        if cur_ans_start < 0:
                            problematic_ids.append(qa.get("id", "missing"))
                            break
                        cur_id = cur_doc.id
                    else:
                        # TODO: Maybe here we do lowercase as well ?
                        cur_ans_start = answer.get('answer_start', cur_doc.text.find(ans))
                        ans_position = cur_doc.text[cur_ans_start: cur_ans_start + len(ans)]
                        # while ans_position.endswith(tuple(string.punctuation)):
                        #     ans_position = ans_position[0: -1]
                        if ans != ans_position:
                            # do not use answer
                            problematic_ids.append(qa.get("id","missing"))
                            break
                        ...
                        if len(splits) == 1:
                            cur_id = splits[0].id
                            cur_ans_start = cur_ans_start
                        else:
                            # TODO: Update the answer_start calculation for the next
                            for s in splits:
